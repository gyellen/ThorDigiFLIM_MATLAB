classdef DecayFitter_exp2_gauss < DecayFitter
    %DecayFitter_exp2_gauss   a lifetime decay fitters
    %   performs nonlinear fit
    
    properties
        params
        floats = logical([1 1 1 1 1 1 1])
        
        cycleLength = 12.5;  % typically 12.5 ns
        bkg = 0;             % inferred background
        
        transforms = [0 0 0 0 0 0 3]; % bounded positive for scatter
    ...
    methods
        function obj = DecayFitter_exp2_gauss
        end
        ...
        function [vals,fail,tau,redchisq] = fitProgressive(obj,t,data,gaussW)
            ...
            p = [mx tau 0 tau/8 tPk0 gaussW 0];
            % respect the fixed parameters, preset only the floats
            obj.params(obj.floats) = p(obj.floats);
            
            % start with a single exponential fit
            [vals,fail] = obj.fit(t,data,[1 1 0 0 0 0 0]);
            if fail, fail=-1; return; end
            if obj.floats(3), obj.params(3)=obj.params(1)*2; end % amp2 starting
            if obj.floats(4), obj.params(4)=obj.params(2)*2; end % tau2 starting longer
            if obj.floats(2), obj.params(2)=obj.params(2)/3; end % adj tau1 to shorter tau
            
            % progressively relax the fit
            [vals,fail] = obj.fit(t,data,[1 1 1 0 1 0 0]);
            if fail, fail=-2; return; end
            [vals,fail] = obj.fit(t,data,[1 1 1 1 1 0 0]);
            if fail, fail=-3; return; end
            [vals,fail] = obj.fit(t,data,[1 1 1 1 1 1 0]);
            if fail, fail=-4; return; end
            [vals,fail,tau,redchisq] = obj.fit(t,data);
            if fail, fail=-5; return; end
        end
        ...
        function tau = tauFitTruncated(obj,tIntegration,params)
            % returns the mean tau from the definite integral from 0 to tIntegration
            %    defintegral of t*F(t) / defintegral of F(t)
            % where F(t) = amp1*exp(-t/tau1) + amp2*(exp(-t/tau2)
            if nargin<2, tIntegration = 8; end  % default is tau<8>
            ...
            xp2    = exp(-tIntegration/tau2);
            tau = (amp1*tau1*(tau1 - xp1*(tIntegration+tau1)) + ...
                amp2*tau2*(tau2 - xp2*(tIntegration+tau2))) / ...
                (amp1*tau1*(1-xp1) + amp2*tau2*(1-xp2));
        end
        ...
        function varargout = fit(obj,t,data,varargin) % optional args: floats, params
            ...
            [vals,bkg] = obj.modelFcn(obj.params, t, data);
            obj.bkg = bkg;
            if nargout<3, varargout = {vals,fail}; return; end
            
            % if requested, compute tauTrunc, chisqreduced
            % OLD WAY:  tau = obj.tauEffectiveTruncated(t,vals);
            % NEW WAY:  integration time 8 ns
            switch obj.tauType
                case 1
                    tau = obj.tauFitTruncated(obj.tauParam(2));
                case 2
                    tau = obj.tauEffectiveWindowed(t,data);
            end
            residual = data(:) - vals(:);
        ...
        function [fit,bkg] = modelFcn(obj, p, t, data) 
            ...
            y1 = amp2*exp(gaussW^2/2/tau2^2 - (t-tPeak)/tau2);
            y2 = erfc((gaussW^2-tau2*(t-tPeak))/(sqrt(2)*tau2*gaussW));
            y=y1.*y2;
            
            y1 = amp2*exp(gaussW^2/2/tau2^2 - (t-tPeak+pulseI)/tau2);
            y2 = erfc((gaussW^2-tau2*(t-tPeak+pulseI))/(sqrt(2)*tau2*gaussW));
